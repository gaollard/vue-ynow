<template>
  <div class="view" v-if="itemInfo">
    ...
    <div class="group group-btm">
      ...
      <router-link class="btn-want" :to="'/chatItem/' + itemInfo.user.id"
        >我想要</router-link
      >
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    doGetXzProductCollectState (typeId) {
      ynowApi
        .getXzProductCollectState({ itemId: this.$route.params.itemId, typeId })
        .then(res => {
          if (typeId === 1) {
    ...
    doCreateCollet (typeId) {
      ...
      ynowApi
        .createXzProductCollect({ itemId: this.$route.params.itemId, typeId })
        .then(res => {
